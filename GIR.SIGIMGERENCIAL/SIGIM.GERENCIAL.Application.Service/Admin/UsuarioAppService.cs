...
namespace GIR.SIGIMGERENCIAL.Application.Service.Admin
{
    public class UsuarioAppService : BaseAppService, IUsuarioAppService
    {
        ...
        public bool SalvarPermissoes(int UsuarioId, int ModuloId, int? PerfilId, List<UsuarioFuncionalidadeDTO> listaFuncionalidadeDTO)
        {
            if (!UsuarioLogado.IsInRole(Funcionalidade.UsuarioFuncionalidadeGravar))
            {
                messageQueue.Add(Resource.Comum.ErrorMessages.PrivilegiosInsuficientes, TypeMessage.Error);
                return false;
            }
            ...
            if (Validator.IsValid(objUsuario, out validationErrors))
            {
                try
                {
                    ...
                    usuarioRepository.UnitOfWork.Commit();
                    messageQueue.Add(Resource.Comum.SuccessMessages.SalvoComSucesso, TypeMessage.Success);
                    return true;
                }
                catch (Exception exception)
                {
        ...
        public bool DeletarPermissoes(int UsuarioId, int ModuloId)
        {
            ...
            if (Validator.IsValid(objUsuario, out validationErrors))
            {
                try
                {
                    usuarioRepository.Alterar(objUsuario);
                    usuarioRepository.UnitOfWork.Commit();
                    messageQueue.Add(Resource.Comum.SuccessMessages.ExcluidoComSucesso, TypeMessage.Success);
                    return true;
                }
                catch (Exception exception)
                {
        ...
        public bool ValidaDeletar(int UsuarioId, int ModuloId)
        {
            ...
            {
                messageQueue.Add(Application.Resource.Comum.ErrorMessages.RegistroNaoRecuperado, TypeMessage.Error);
                return false;
            }
        ...
        public bool ValidaUsuario(int UsuarioId)
        {
            if (UsuarioId == 0)
            {
                messageQueue.Add(string.Format(Resource.Comum.ErrorMessages.CampoObrigatorio, "Usuário"), TypeMessage.Error);
                return false;
            }
        ...
        public bool ValidaModulo(int ModuloId)
        {
            if (ModuloId == 0)
            {
                messageQueue.Add(string.Format(Resource.Comum.ErrorMessages.CampoObrigatorio, "Módulo"), TypeMessage.Error);
                return false;
            }
